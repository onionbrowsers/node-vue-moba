module.exports = app => {
    ...
    router.get('/news/list', async (req, res) => {
        // 选择分类表中名字为新闻资讯的，并且向其中填充children属性，向children属性中填充newsList属性
        // const parent = await Category.findOne({
        //     name: '新闻资讯'
        // }).populate({
        //     path: 'children',
        ...
        const cats = await Category.aggregate([
            // 匹配新闻资讯document中含有parent字段的document
            {
                $match: {
                    parent: parent._id
                }
            ...
            {
                $lookup: {
                    // from要查表的名称，localField本地键字段名，foreignField外键要查的字段名，as两者关联返回的自定义名称
                    from: 'articles',
                    localField: '_id',
            ...
            {
                // 自定义加什么属性，因为上述已经定义newsList，所以这里就是修改newsList，使用slice方法，只要前5个
                $addFields: {
                    newsList: {
                        $slice: ['$newsList', 5]
                    }
        ...
        cats.unshift({
            name: '热门',
            newsList: await News.find().where({
                categories: {
                    // $In操作符，选择categories中的5个id对应的categories中的数据
                    $in: cats.map(v => v._id)
                }
            }).populate('categories').limit(5).lean()
        })
        // 给要返回的数据加上categoryName
        cats.map(cat => {
            cat.newsList.map(news => {
                news.categoryName = cat.name !== '热门' ? cat.name : news.categories[0].name
                return news
            })
            return cat
        })
    ...
    router.get('/heros/init', async (req, res) => {
        ...
        for (let cat of rawData) {
            ...
            cat.heros = cat.heros.map(hero => {
                // 给英雄加上标签，比如属于射手，辅助这一类，因为是多选，所以是数组
